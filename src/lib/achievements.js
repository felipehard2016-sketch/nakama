...
/* ─────────────────────────────────────────────────────────────
   Definições das 27 conquistas (herdadas quase inteiras da v1 —
   a regra de desbloqueio é a mesma, só a fonte dos dados mudou:
   agora vem de user_media_list/media_items (v2) em vez de localStorage).
   check(stats) → { unlocked: bool, progress: number, total: number }
───────────────────────────────────────────────────────────── */

export const ACHIEVEMENT_CATEGORIES = ['Coleção', 'Maratona', 'Variedade', 'Dedicação'];

export const ACHIEVEMENTS = [
  /* ── Coleção ── */
  { id: 'first_add', icon: '🌱', category: 'Coleção', points: 10, title: 'Primeira Adição',
  ...
  { id: 'hours_240', icon: '🕰️', category: 'Maratona', points: 100, title: 'Dez Dias',
  ...
  /* ── Variedade ── */
  { id: 'genres_5', icon: '🎭', category: 'Variedade', points: 30, title: 'Eclético',
  ...
  { id: 'early_adopter', icon: '🎌', category: 'Dedicação', points: 30, title: 'Nakama OG',
    desc: 'Um dos primeiros usuários do Nakama.',
    check: () => ({ unlocked: true, progress: 1, total: 1 }) }, // todos desbloqueiam
];

/** Calcula as stats de conquista a partir da lista (getUserList) + streak (getStreak) do usuário. */
export function computeAchievementStats(entries = [], streak = null) {
  const completed = entries.filter(e => e.status === 'completed');
  const animes = entries.filter(e => e.media_items?.type === 'anime');
  const mangas = entries.filter(e => e.media_items?.type === 'manga');

  const totalEps   = animes.reduce((s, e) => s + (e.progress || 0), 0);
  const totalChaps = mangas.reduce((s, e) => s + (e.progress || 0), 0);
  const totalMin   = animes.reduce((s, e) => s + (e.progress || 0) * (e.media_items?.metadata?.duration || 24), 0)
                    + mangas.reduce((s, e) => s + (e.progress || 0) * 5, 0);

  const allGenres = new Set(entries.flatMap(e => e.media_items?.metadata?.genres || []));
  const completedGenreSet = new Set(completed.flatMap(e => e.media_items?.metadata?.genres || []));
  ...
  return {
    ...
    movieCount:      entries.filter(e => e.media_items?.metadata?.format === 'MOVIE').length,
    scoredCount:     entries.filter(e => e.rating != null).length,
...
export function checkAllAchievements(entries, streak) {
  const stats = computeAchievementStats(entries, streak);
  return ACHIEVEMENTS.map(a => ({ ...a, ...a.check(stats) }));
}
...
/** Grava no banco (user_achievements) as conquistas recém-desbloqueadas que ainda não estavam lá. */
export async function syncUnlockedAchievements(userId, checked, alreadyUnlockedIds) {
  const newlyUnlocked = checked.filter(a => a.unlocked && !alreadyUnlockedIds.has(a.id));
  if (newlyUnlocked.length === 0) return [];

  const { error } = await supabase
    .from('user_achievements')
    .upsert(newlyUnlocked.map(a => ({ user_id: userId, achievement_id: a.id })), { onConflict: 'user_id,achievement_id' });

  return error ? [] : newlyUnlocked;
}

export async function getUnlockedAchievementIds(userId) {
